 ...
 */
public class BlockTailor extends BlockContainer {
   ...
   public boolean renderAsNormalBlock()
   {
    ...
    {
    	boolean completeStack = slot < 4 || player.isSneaking();
    	ItemStack heldItem = player.getHeldItem();
    	ItemStack slotItem = tile.getStackInSlot(slot);
    	
    	if(heldItem != null && heldItem.getItem() instanceof INeedle)
    	{
    		return false;
    	}
    ...
    public static boolean useInventory(World world, int x, int y, int z, TileEntityTailor tile, EntityPlayer player, int i, int slot) 
    {
        ...
        if (i != 1)
        {
           player.openGui(MineFantasyBase.instance, 3, world, x, y, z);
        }
    	return false;
    }
}
