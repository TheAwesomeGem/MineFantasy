...
public class BlockClickHelper
{
	...
	public static int[] getCoordsFor(float clickX, float clickY, float xBound, float xBound2, float yBound, float yBound2, int xSlots, int ySlots, int dir) 
	{
		...
		float xSpace = xMax / (float)xSlots;
		float ySpace = yMax / (float)ySlots;
		...
		return translateCoords(xSlot, ySlot, xSlots, ySlots, dir);
	}
	...
	{
		ForgeDirection dir = FD[direction];
	...
	public static ForgeDirection[] FD = new ForgeDirection[]{ForgeDirection.SOUTH, ForgeDirection.WEST, ForgeDirection.NORTH, ForgeDirection.EAST};
}
