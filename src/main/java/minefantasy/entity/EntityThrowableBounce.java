...
public abstract class EntityThrowableBounce extends Entity implements IProjectile
{
    private int xTile = -1;
    private int yTile = -1;
    public int ticksRemaining = getFuseTime();
    private int zTile = -1;
    ...
    public EntityThrowableBounce(World par1World, EntityLivingBase par2EntityLivingBase)
    {
        ...
        this.setThrowableHeading(this.motionX, this.motionY, this.motionZ, getThrownPower(), 1.0F);
    }

    /**
     * Gets the power of the entity when thrown
     * @return Default 1.5
     */
    ...
    public void onUpdate()
    {
        ...
        if(motionY < 0.01 && motionY > -0.01 && this.onGround)
		{
        	float drag2 = 0.8F;
        	motionX *= drag2;
        	motionY *= drag2;
        ...
        if (var3 != null)
        {
            ...
            {
        		int id = this.worldObj.getBlockId(var3.blockX, var3.blockY, var3.blockZ);
        		if(id != 0)
        		{
        			...
        			if(block != null)
        			{
        				...
        				{
        					System.out.println("NonSolid: " + block.blockMaterial.toString());
                			bounce = false;
        				}
        				...
        				if(shouldBreakBlock(block))
        				{
        					...
        					bounce = false;
        					this.worldObj.spawnParticle("tilecrack_" + id + "_" + this.worldObj.getBlockMetadata(var3.blockX, var3.blockY, var3.blockZ), var3.blockX + 0.5D, var3.blockY + 0.5D, var3.blockZ + 0.5D, 0D, 0D, 0D);
        				}
        			}
        		}
            }
    ...
    protected void bounce(MovingObjectPosition var1) {
		...
		if(resistance > 0.2F)
		worldObj.playSoundAtEntity(this, data_minefantasy.sound("Weapon.bombBounce"), resistance+0.2F, 0.75F);
		if(var1.sideHit == 0 && motionY > 0)//BOTTOM
		{
			motionY *= -resistance;
			if(motionY < 0.01 && motionY > -0.01 && this.onGround)
			{
		...
		if(var1.sideHit == 1 && motionY < 0)//TOP
		{
			motionY *= -resistance;
			if(motionY < 0.01 && motionY > -0.01 && this.onGround)
			{
		...
		if(var1.sideHit == 2 && motionZ > 0)//EAST
		{
			motionZ *= -resistance;
		}
		if(var1.sideHit == 3 && motionZ < 0)//WEST
		{
			motionZ *= -resistance;
		}
		
		if(var1.sideHit == 4 && motionX > 0)//NORTH
		{
			motionX *= -resistance;
		}
		if(var1.sideHit == 5 && motionX < 0)//SOUTH
		{
			motionX *= -resistance;
		}
		if(var1.entityHit != null)
		{
	        ...
	        {
	        	var1.entityHit.attackEntityFrom(DamageSource.causeThrownDamage(this, thrower), 1);
	        }
		}
	}
    
    /**
     * Gets the amount of energy reduced on bouncing
     * @return the resulting energy as a decimal
     */
    ...
    public void writeEntityToNBT(NBTTagCompound par1NBTTagCompound)
    {
        ...
        par1NBTTagCompound.setInteger("fuse", ticksRemaining);
    ...
    public void readEntityFromNBT(NBTTagCompound par1NBTTagCompound)
    {
        ...
        this.ticksRemaining = par1NBTTagCompound.getInteger("fuse");
    ...
    public abstract void explode(MovingObjectPosition pos);
