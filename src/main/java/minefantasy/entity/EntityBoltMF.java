...
public class EntityBoltMF extends EntityArrow implements IProjectile, PacketUserMF
{
    ...
    /** 1 if the player can pick up the Bolt */
    public int canBePickedUp = 0;

    /** Seems to be some sort of timer for animating an Bolt. */
    public int BoltShake = 0;

    /** The owner of this Bolt. */
    public Entity shootingEntity;
    private int ticksInGround;
    private int ticksInAir = 0;

    /** The amount of knockback an Bolt applies when it hits a mob. */
        ...
        else
        {
            ...
            if (movingobjectposition != null)
            {
                if (movingobjectposition.entityHit != null && !hasHit)
                {
                    ...
                    if(!worldObj.isRemote && MineFantasyBase.isDebug())
                    {
                    	System.out.println("Base Damage: " + dam);
                    }
                    ...
                    if(!worldObj.isRemote && MineFantasyBase.isDebug())
                    {
                    	System.out.println("Full Damage: " + dam);
                    }
    ...
    {
    	hasHit = true;
    	if(this.getBolt() != null)
    	{
    		int c = getBolt().getBreakChance();
    		int chance = -1;
    		if(c != -1)
    		{
				if(c <= 0)c = 1;
    		...
    		{
    			System.out.println("Arrow Break Chance: 1 in " + getBolt().getBreakChance() + ". hit has rolled a " + chance);
    		}
    ...
    public void writeEntityToNBT(NBTTagCompound nbt)
    {
        ...
        nbt.setByte("shake", (byte)this.BoltShake);
    ...
    public void readEntityFromNBT(NBTTagCompound nbt)
    {
        ...
        this.BoltShake = nbt.getByte("shake") & 255;
    ...
    public void onCollideWithPlayer(EntityPlayer player)
    {
        ...
        if (drop != null && !this.worldObj.isRemote && this.inGround && this.BoltShake <= 0)
        {
    ...
    {
    	return BoltType.getDamage(type) * super.getDamage();
    }
    /**
     * Sets the amount of knockback the Bolt applies when it hits a mob.
     */
    ...
    /**
     * Whether the Bolt has a stream of critical hit particles flying behind it.
     */
    ...
    {
    	this.type = BoltType.getBolt(type);
	...
	{
		return new ItemStack(ItemListMF.boltMF, 1, getType());
	}
	...
	{
		return BoltType.getBolt(getType());
	}
}
